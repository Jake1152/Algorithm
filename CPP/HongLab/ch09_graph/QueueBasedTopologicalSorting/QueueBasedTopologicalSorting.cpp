...
public:
	...
	stack<Vertex*> QueueBasedTopologicalSort()
	{
		...
		// TODO:
		for (size_t idx = 0; idx < this->vertices.size(); idx++)
			indegree[idx] = this->vertices[idx]->in_neighbors.size();
		...
		for (size_t idx = 0; idx < this->vertices.size(); idx++)
			if (indegree[idx] == 0)
				q.push(this->vertices[idx]);

		vector<Vertex*> result;

		/**
		 * 진입차수가 0인 정점의 진출 노드들의 진입차수를 1개씩 감소시킨다
		 * 진출노드들의 진입차수가 0이면 result에 추가한다.
		 * 
		*/
		// TODO:
		while (q.empty() == false)
		{
			Vertex *cur_node = q.front();

			cout << "# cur_node->value : " << cur_node->value << endl;

			q.pop();
			for (Vertex *out_neighbor : cur_node->out_neighbors)
			{
				indegree[out_neighbor->value] -= 1;
				if (indegree[out_neighbor->value] == 0)
					q.push(out_neighbor);
			}
			/**
			 * 왜 이 조건은 동작하지 않았는가?
			 * 진출차수가 없지만 마지막일 수도 있는가?
			*/
			// 진입차수가 0이고
			// 진출차수가 있거나 진출차수가 없다면 마지막노드 일때만 추가
			// if (cur_node->out_neighbors.size() > 0 \
			// 			|| result.size() + 1 ==  this->vertices.size()
			// 	)
