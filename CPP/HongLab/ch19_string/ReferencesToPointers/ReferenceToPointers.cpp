...
{
	cout << "Before assign f in Test1(): " << f << endl;
	cout << "Before assign &f in Test1(): " << &f << endl;
	f = new float(123.0f);
	cout << "f in Test1(): " << f << endl;
	cout << "*f in Test1(): " << *f << endl;
}

void Test2(float*& f) // 매개변수 f의 자료형이 포인터의 참조(float*&)
{
	cout << "Before assign f in Test2(): " << f << endl;
	cout << "Before assign &f in Test2(): " << &f << endl;
	f = new float(456.0f);
	cout << "f in Test2(): " << f << endl;
	cout << "*f in Test2(): " << *f << endl;
}

void Test3(float* * const f) 
{
	cout << "Before assign *f in Test3(): " << *f << endl;
	cout << "Before assign f in Test3(): " << f << endl;
	*f = new float(456.0f);
	cout << "*f in Test3(): " << *f << endl;
	cout << "**f in Test3(): " << **f << endl;
}
...
int main()
{
	...
	cout << f1 << endl << endl; // f1은 여전히 nullptr라서 0000000000000000 출력

	float* f2 = nullptr;

	Test2(f2);

	cout << f2 << " " << *f2 << endl << endl; // 0000017E006E67B0 (주소는 달라질 수 있음) 456 <- Test2() 안에서 만들어준 대로 출력
